...
% Rate - sampling rate
function Out = SlidingPercentileVelocity(Data, VelWindow, VelOverlap, Rate, quantile,USEBGR,bgr,numQuads,short_term_buffer_length)
...
for j = 1:(WindowSamples-OverlapSamples):(N + 1 - WindowSamples)
    ...
    if j==1
        phaseDiff(k) = abs(Range(WindowSamples)-Range(1));
    else

function [Results] = GenerateCrossEnvironmentResults_Nontarget_IoTDI(path_models, path_train_envs, path_test_envs)
...
nontarget_set{1} = [1 2 3 10]; % Ball set
nontarget_set{2} = [4 6]; % Car set
nontarget_set{3} = [5 7 8 9]; % Dog set
...
for i=1:length(trainFiles) % take every file from the set 'Files'
    ...
    for j=1:length(testFiles)
        ...
        % Ignore test environment if same as train environment, or is a constituent of the (composite) train environment
...
%         end
        ...
        Results = [Results; {fileName}, {testFileName}, accuracy];
    end
end

function [model_outpath, return_value, result, confusionmatrix]=Crossenv_val_new(path_models,full_arff_folder,fileName, testEnvName, c, gamma, generate_model, return_type, compIndex)       % used when trim data is used. Crossval is used when all the data is used(do not let frames in the same file showing in both training and testing set)
...
instances = loadARFF(sprintf('%s/%s',full_arff_folder,[fileName,'.arff']));
testFile = loadARFF(sprintf('%s/%s',full_arff_folder,[testEnvName,'.arff']));
...
switch lower(return_type)
    case 'precision'
        return_value = eval.precision(compIndex);
    case 'recall'
        return_value = eval.recall(compIndex);
    case 'truepositiverate'
        return_value = eval.numTruePositives(compIndex);
    case 'falsepositiverate'
        return_value = eval.numTruePositives(compIndex);
    case 'truenegativerate'
        return_value = eval.numTrueNegatives(compIndex);
    case 'falsenegativerate'
        return_value = eval.numFalseNegatives(compIndex);
    otherwise
        return_value=eval.pctCorrect(); % default: accuracy
end

...
%  follow the link http://www.mathworks.com/matlabcentral/answers/103056-how-do-i-change-the-java-virtual-machine-jvm-that-matlab-is-using
    ...
    if(iscell(data))
        for i=1:numel(featureNames)
            %if(i == numel(featureNames) && ischar(data{1,i}))
            if(ischar(data{1,i}))
                attvals = unique(data(:,i));
                values = FastVector();
                fprintf('Finding unique values for Feature number %d\n',i);
...
%                 end
            ...
            elseif(i == numel(featureNames)-1 && ischar(data{1,i}))
                fprintf('Processing Feature number : %d\n',i);
                %attvals = unique(data(:,i));
                %values = FastVector();
                %values.addElement(attvals{1});
                %bool = javaObject('Boolean("True")');
                %bool = java.lang.Boolean('True');
                %fprintf(bool.booleanValue());
                vec.addElement(Attribute(featureNames{i},int16.empty(5,0)));
            else
    ...
    if(iscell(data))
        for i=1:size(data,1)
            ...
            %for j = numel(featureNames)-2:numel(featureNames)-1
            if ifReg==1
                inst.setValue(j,data{i,j+1});
            else
                if data{i,j+1}<=9
                    fprintf('Setting value to featurenum %d\n',j+1);
                    %data{i,j+1}
                    inst.setValue(j,Character.toString(num2str(data{i,j+1})));
                    %inst.setValue(j+1,Character.toString(num2str(data{i,j+2})));
                else
                    fprintf('Setting value to featurenum %d\n',j+1);
                    %data{i,j+1}
                    inst.setValue(j,num2str(data{i,j+1}));
                    %inst.setValue(j+1,num2str(data{i,j+2}));
                end
            end   
            wekaOBJ.add(inst);
            %end

function [crossval_mRMRMAD_D_norm, crossenvironment_mRMRMAD_D_norm] = Results_Sensys_mRMRMAD_D_BigEnvs(arff_folder, alpha, round, top_k, heldout_or_single)
...
for i=1:length(testFiles) % take every file from the set 'Files'
    ...
    outfileName = strcat(path_to_mRMRMAD_D_test,'/',testFiles{i},'_', mRMRMAD_D_features_file_suffix,'.arff');
    AttributeSelectionManual_Arff(infileFullName, outfileName, mRMRMAD_D_features_csv);
end
...
fprintf('--------------------------------------------\n');
crossval_mRMRMAD_D_norm = GenerateModels_IoTDI(g_str_pathbase_model, path_to_mRMRMAD_D_arffs);
fprintf('--------------------------------------------\n\n');
...
fprintf('-------------------------------------------------------------------\n');
crossenvironment_mRMRMAD_D_norm = GenerateCrossEnvironmentResults_HumanOnly_IoTDI(g_str_pathbase_model, path_to_mRMRMAD_D_arffs, path_to_mRMRMAD_D_test);
fprintf('-------------------------------------------------------------------\n\n');

%{ 
 ...
 %}
function sanityCheckOSC(hrs, minute, second, beg_file,end_file,output_file,window)
%SetEnvironment;
%SetPath;
...
start_time = datetime(cur_year, cur_month, cur_day, hrs, minute, second,prefix);
disp(start_time);

%path to the files containing start and end times from the cuts
file_path_beginnings = strcat(prefix,beg_file);
file_path_ends = strcat(prefix,end_file);
%disp(file_path);
%disp(length(files));
...
%disp(start_arr)
output_file_path = strcat(prefix,strcat(output_file,'.csv'));
